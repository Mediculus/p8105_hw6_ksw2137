...
Using `broom::glance()` shows that our adjusted R-squared is at a pretty good value of `r round(summary(first_fit_lm)["adj.r.squared"], 3)`, which means `r round(summary(first_fit_lm)["adj.r.squared"], 3) * 100`% of the variability in `bwt` is explained by our covariates. 
...
We see from our coefficient summary that each variable have a significant p < 0.05 except for Asian mothers (White as the reference race)
...
birthweight_df %>% 
  add_residuals(first_fit_lm) %>% 
  add_predictions(first_fit_lm) %>% 
...
We will use cross validation in order to compare our models with these two by looking for the lowest RMSE. 
...
trained_cv_df <- trained_cv_df %>% 
  mutate(
    rmse_prop = map2_dbl(prop_lm, test, ~rmse(model = .x, data = .y)),
    rmse_mod1 = map2_dbl(mod1_lm, test, ~rmse(model = .x, data = .y)),
    rmse_mod2 = map2_dbl(mod2_lm, test, ~rmse(model = .x, data = .y))
    )
...
clean_weather_strap <- bootstrap_runs %>% 
